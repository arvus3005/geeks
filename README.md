...
### 2026-08-22, evening (continued) — A leaked-then-rotated credential, a Sanskrit retry, and closing the disk-cleanup loop

**A real API key landed in a chat transcript.** An Anthropic key was pasted directly into the working conversation instead of added to `.env` as asked — treated as compromised regardless of handling (a transcript is a transcript), key stored in `.env` and as a GitHub Actions secret (`gh secret set`, verified via `gh secret list`), and rotation was recommended to the user rather than assumed unnecessary just because this project's own handling was careful.

**Two more build-time-only artifacts regenerated and were cleaned again**: `artifacts/full_index_dedup` (667MB, the same dedup-tracking SQLite category removed this morning — regenerated by today's new as/kn/ml/or/pa/sa build attempts) and a previously-unnoticed `artifacts/local_index` (256MB) — confirmed via grep to have zero code references and confirmed `settings.local_index_root` points at `full_local_index` instead, so this was leftover from an earlier non-sharded prototype, not anything live. Also cleared `.mypy_cache`/`.pytest_cache`/`.ruff_cache`/`.hypothesis`/`.cache` (pure regenerating tool/download caches, zero risk).

**Sanskrit (`sa`) needed three attempts, and landed as a genuinely usable pilot.** First attempt hit the 15GB free-disk floor at just 2,000/~700k+ rows in (nothing finalized, deleted). Second attempt, after the cleanup above freed headroom, got to 4,000/5,000 rows before hitting the same floor again — still no finalized segment. Third attempt lowered the row cap to 3,500 (a target the remaining headroom could actually guarantee finishing) and it finalized cleanly: 69,458 real passages, `"status": "complete"`. Wired into routing the same way as the other pilots — Sanskrit is also written in Devanagari, so it joins the same 4-way script-ambiguity group as hi/mr/ne (verified live: a real Sanskrit query correctly fans out to all four shards). A real regression test now covers it, same as every other language added today — this project has been bitten before by adding a script range without wiring it all the way through (see the Gurmukhi entry above), so each new language gets a live functional check plus a test, not just a routing-table edit.

**A leaked-key lesson on top of the leaked-key lesson**: the pasted Anthropic key turned out to authenticate fine but have zero account credit. Worse than a missing key: `eval/judge.py` only gracefully reports "SKIPPED" when the key is *absent* — an invalid-credit key still gets called, gets a real 400 from Anthropic, and that exception isn't caught anywhere up the call stack, crashing the entire eval run (losing the other 3 checks' results too, not just Faithfulness/Correctness). Commented the key out in `.env` (kept, not deleted) until real credits exist, restoring the graceful-skip path.

**Investigated whether there's a real optimization left in the eval's non-judge checks, found there mostly isn't.** The Retrieval check (Recall@1/3/5, MRR) turned out to test something narrower than assumed: it builds its own FAISS index and scores pure embedding-cosine-similarity search only — no BM25, no RRF, no reranker, none of the actual production hybrid pipeline touches this number at all. Checked for a real, known e5-model pitfall (missing `"query: "`/`"passage: "` prefix, which silently degrades quality) — already correctly implemented. Reliability's false-refusal rate (2.0%) and Latency (both stages already PASS) have little realistic headroom left either. Reported honestly rather than forcing a speculative change with no real evidence behind it.

**Also removed `docs/EVAL_LOOP_TARGET_INTERFACE.md`** (the eval suite's own contract spec, copied in earlier) once it became clear the same information already lives in `app/embedder.py`'s and `app/generator.py`'s own docstrings — a standalone copy was redundant reference material, not anything unique.
...
| Self-hosted indexing | ✅ 6/14 Indic configs at full corpus (+ the shared English pool, 54.25M passages, integrity-verified) + 7/14 more at pilot scale (`ne`/`as`/`kn`/`ml`/`or`/`pa`/`sa`, ~50-100k passages each, honestly labeled per CLAUDE.md — see `/v1/system`) = 13/14 configs with real servable data. Only `te` unbuilt (no train split upstream, not pursued). Live serving from this index (no managed vector DB) |
...
- [`docs/wiring-in-the-eval-loop.pdf`](docs/wiring-in-the-eval-loop.pdf) — the hackathon's own runbook for `rag-local-eval-loop`, saved verbatim. `eval/` + `run.sh` in this repo's root are the actual suite, copied in per that runbook rather than left external — see `app/embedder.py` / `app/generator.py`'s own docstrings for the exact target-interface contract they satisfy, and `results/` for real run output.
