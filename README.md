...
1. **Voice & Speech-to-Text (`SarvamSTTAdapter` + Whisper fallback)**: Sarvam AI API for Indic speech recognition. Cross-checked 2026-08-22 against the real language map (`SARVAM_STT_LANG_MAP` in `src/hhgoa_rag/stt/sarvam.py`), not assumed: STT is wired for **hi, bn, gu, mr, ta, ur, te, kn, ml, pa, or(Odia)** — notably **not** `as`/`ne`/`sa`, despite all three being in the indexed corpus (a voice query in Assamese, Nepali, or Sanskrit gets no explicit language hint sent to Sarvam, falling back to its auto-detect), and `te` (Telugu) has STT wired despite having zero corpus to answer from. A real bug was found and fixed in the same pass: the map's key used to be Sarvam's own wire code `"od"` instead of this project's internal code `"or"` (used everywhere else — `language_routing.py`), so an explicit `language_hint=or` API call silently fell through to `"unknown"` instead of `"od-IN"`. Not reachable through the shipped web UI (its language dropdown doesn't offer Odia at all), so it was latent rather than live-breaking, but real for any direct API caller — the identical bug existed in the TTS adapter's own language map (`SARVAM_LANG_MAP` in `src/hhgoa_rag/stt/tts.py`, would have spoken Odia responses in English), fixed the same way. Both covered by new tests (`tests/unit/test_sarvam_retry.py`, `tests/unit/test_tts_payload.py`).
