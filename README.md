...
| **4. Latency Target** | Full pipeline through to final answer under **200 ms** | **Verified live against the deployed Render instance** (`https://hhgoa-rag-d3fw.onrender.com`, Ohio region), 32 real queries, server-side pipeline time (`total_backend_ms`, i.e. STT-excluded end-to-end processing — chunking is offline, per CLAUDE.md): **P50 42.9ms, P70 47.8ms, P95 100.0ms, P100 151.6ms** — under budget at every percentile. Deploying in Render's Ohio region (near Pinecone's `aws/us-east-1`) cut the Pinecone round-trip from ~292ms (measured pre-deploy, from a dev sandbox far from that region) to **31.3ms**, confirming the earlier network-bound diagnosis was correct and fixable by region placement alone — no further compute optimization was needed. See `artifacts/reports/latency_benchmark_20260819T205335.{json,md}`. | **MEASURED — UNDER 200ms AT EVERY PERCENTILE** |
...
| **Live Benchmark (P50/P70/P100)** | Latency numbers from a real run, not best-case | **Done — server-side P50 42.9ms / P70 47.8ms / P95 100.0ms / P100 151.6ms**, under the 200ms target at every percentile, measured against the live deployment; committed in `artifacts/reports/` |
