...
1. **Voice & Speech-to-Text (`SarvamSTTAdapter` + Whisper fallback)**: Sarvam AI API for Indic speech recognition. Cross-checked 2026-08-22 against the real language map (`SARVAM_STT_LANG_MAP` in `src/hhgoa_rag/stt/sarvam.py`), not assumed: STT is actually wired for **hi, bn, gu, mr, ta, ur, te, kn, ml, pa, od(Odia)** — notably **not** `as`/`ne`/`sa`, despite all three being in the indexed corpus (a voice query in Assamese, Nepali, or Sanskrit gets no explicit language hint sent to Sarvam, falling back to its auto-detect), and `te` (Telugu) has STT wired despite having zero corpus to answer from. Also found: the map's key is `"od"` but this project's own internal code for Odia is `"or"` everywhere else in the codebase — an explicit `language_hint=or` API call would silently fall through to `"unknown"` instead of `"od-IN"`. Not reachable through the shipped web UI (its language dropdown only offers hi/bn/mr/gu/ta/ur/en, no Odia option), so latent rather than live-breaking, but a real gap worth fixing in the STT code itself, not just documenting here.
